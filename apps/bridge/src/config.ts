...
export const CUPCAT_VERSION = process.env.CUPCAT_VERSION ?? "1.7.28";
...
/** Where releases are read from. Overridable only so an update can be rehearsed end to end against a
 * local server — downloading and swapping real binaries — instead of being trusted on first contact
 * with a real user's machine. */
export const GITHUB_API = process.env.CUPCAT_GITHUB_API ?? "https://api.github.com";
